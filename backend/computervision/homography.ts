 ...
 */
function getCornerPoint(cornerMat: any, index: number): Point {
  return {
...
function detectMarkers(): void {
  ...
  if (ids.rows > 0) {
    ...
    if (homoPts) {
      ...
      status.textContent = "Homography active";
    } else {
      ...
      status.textContent = `Waiting for markers: ${needed.join(", ")}`;
    }
  } else {
